...
fs.readdir("./src/commands", (err, files) => {
  ...
  console.log(`(!) Carregando ${files.length} comandos do bot`);
  files.forEach((f) => {
...
client.on("messageCreate", async (message) => {
  ...
  if (cmd) cmd.run(client, message, params, p);
});
/*=======================================================================================*/
...
client.on("ready", async () => {
  console.log(
    "(!) Bot " + client.user.tag + " logado e configurado com sucesso!"
  );
  let botsSchema = require("./src/database/models/botlist/bots.js");
  const bots = await botsSchema.find();
  client.user.setPresence({
    activity: { type: "WATCHING", name: "" + bots.length + " Bots" },
    status: "dnd",
  });
});
/*=======================================================================================*/
...
client.on('ready', () => {
  let channel = client.channels.cache.get("1016802236128571422")
...
client.on("guildMemberRemove", async (member) => {
  ...
  await find.forEach(async (b) => {
    member.guild.members.kick(b.botID);
