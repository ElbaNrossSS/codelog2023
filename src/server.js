...
module.exports = async (client) => {
            ...
            if (req.isAuthenticated()) {
                ...
                if (usercheck) {
                    if (usercheck.roles.cache.get(roles.yonetici)) {
                        ...
                        res.redirect('/error?code=200&message=Nosso site está temporariamente indisponivel.')
                    }
                } else {
                    res.redirect('/error?code=200&message=Nosso site está temporariamente indisponivel.')
                }
    ...
    app.get("/callback", passport.authenticate("discord", {
        failureRedirect: "/error?code=999&message=Encontramos um erro ao conectar ao sistema. Tente novamente mais tarde"
    }), async (req, res) => {
...
.setColor(0x0099FF)
.setDescription(`[**${a.username}**#${a.discriminator}](https://codelog.glitch.me/user/${a.id}) logged into the site.`)
.addFields({name:"Username",value: a.username})
.addFields({name:"User ID",value: a.id})
.addFields({name:"User Discriminator", value: a.discriminator})
client.channels.cache.get(channels.login).send({ embeds: [embed2] });
            })
            req.session.destroy(() => {
                res.json({
                    login: false,
                    message: "You have been blocked from Codelog.",
                    logout: true
                })
                req.logout();
            });
...
.setColor(0x0099FF)
.setDescription(`[**${a.username}**#${a.discriminator}](https://codelog.glitch.me/user/${a.id}) Just Logged into the site.`)
.addFields({name:"Username",value: a.username})
.addFields({name:"User ID", value: a.id})
.addFields({name:"User Discriminator",value: a.discriminator})
				.setAuthor({ name:a.username, value:a.avatarURL({dynamic: true})})
				.setThumbnail(a.avatarURL({dynamic: true}))
...
client.channels.cache.get(channels.login).send({ embeds: [embed] });
    ...
    app.get("/dc", (req, res) => {
        res.redirect('https://discord.gg/36EVSeqCeS');
    });
    ...
    app.get("/robots.txt", function(req, res) {
        ...
        res.send(`Sitemap: https://codelog.glitch.me/sitemap.xml`);
    });

    app.get("/sitemap.xml", async function(req, res) {
        let link = "<url><loc>https://codelog.glitch.me/</loc></url>";
        let botdataforxml = await botsdata.find()
        botdataforxml.forEach(bot => {
            link += "\n<url><loc>https://codelog.glitch.me/bot/" + bot.botID + "</loc></url>";
        })
...
const embed3 = new Discord.EmbedBuilder()
                    ...
                    .setColor(0x0099FF)
                  .addFields({name:"User;",value: `${a.tag} \`(${a.id})\` `,inline: true })
                  .addFields({name:"Uptime Code;",value: updcode,inline: true})
                  .addFields({name:"Uptime Limit;",value: `${dd.length+1}/10`,inline: true})
...
client.channels.cache.get(channels.uptimelog).send({ embeds: [embed3] });
    ...
    app.post("/bot/:botID/vote", checkMaintence, checkAuth, async (req, res) => {
        ...
        if (x) return res.redirect("/error?code=400&message=You can vote every 5 hours.");
        ...
        return res.redirect(`/bot/${req.params.botID}/vote?success=true&message=You voted successfully. You can vote again after 5 hours.`);
        renderTemplate(res, req, "botlist/vote.ejs", {
            req,
            roles,
            config,
    ...
    app.post("/addbot", checkMaintence, checkAuth, async (req, res) => {
        ...
        client.users.fetch(rBody['botID']).then(a => {
            client.channels.cache.get(channels.botonay).send(`<@${req.user.id}> bot isteği geldi  **${a.tag}** <@&${channels.botonayrol}>`)
    ...
    app.post("/certification/apply", checkMaintence, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.botonay).send(`User **${req.user.username}** requested a certificate for her bot named **${botdata.username}**. <@&${channels.botonayrol}>`)
    });
    //
...
.setFooter({text: config.footer.text})
            ...
            .setDescription(`The user named **[${req.user.username}](https://codelog.glitch.me/user/${req.user.id})** added the code named **${rBody['codename']}** to the system.`)
          .addFields({name:"Code Link", value:`https://codelog.glitch.me/code/${kod}`,inline: true})
          .addFields({name:"Code Description", value: rBody['codedesc'],inline: true})
          .addFields({name:"Code Category", value:rBody['category'],inline: true})

client.channels.cache.get(channels.codelog).send({ embeds: [embed4] });
...
.setColor(0x0099FF)
.setFooter({text: config.footer.text})
            .setDescription(`The user named **[${req.user.username}](https://codelog.glitch.me/user/${req.user.id})** edited the code named **${rBody['codename']}**.`)
            .addFields({ name:"Code Link",value: `https://codelog.glitch.me/code/${kod}`,inline: true})
            .addFields({ name:"Code Description",value: rBody['codedesc'],inline: true})
            .addFields({name: "Code Category",value: rBody['category'],inline: true})

  client.channels.cache.get(channels.codelog).send({ embeds: [embed5] });
    ...
    app.post("/admin/maintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.webstatus).send(`Codelog has been switched to __maintance__ due to **${req.body.reason}**`).then(a => {
            new maintenceSchema({
                server: config.server.id,
...
const bakimsonaerdikardesTopic = new EmbedBuilder()
           .setAuthor({ name: "Code Log", iconURL:client.user.avatarURL()})
            .setThumbnail(client.user.avatarURL())
           .setColor("Green")
            .setDescription(`Codelog are **active** again!\n[Click to redirect website](https://codelog.glitch.me)`)
   .setFooter({text: config.footer.text})

        await client.channels.cache.get(channels.webstatus).messages.fetch(bakimdata.bakimmsg).then(a => {
a.edit({ content: "", embeds: [bakimsonaerdikardesTopic] })
        })
    ...
    app.get("/user/:userID", checkMaintence, async (req, res) => {
        client.users.fetch(req.params.userID).then(async a => {
            ...
            const botdata = await botsdata.find()
            const member = client.guilds.cache.get("971823165351280685").members.cache.get(a.id)
